...
uint256 hashGenesisBlock("0xb19f7a9a051ad5a1ad5f214cdd71bdf1e93978934e653d32e31164d5b47b7bab");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitblocks: starting difficulty is 1 / 2^12
...
{
    // Bitblocks: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 90 days
    nSubsidy >>= (nHeight / 259200); // Bitblocks: 259.2k blocks in ~90 days

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.25 * 24 * 60 * 60; // Bitblocks: 6 Hours
static const int64 nTargetSpacing = 0.5 * 60; // Bitblocks: 0.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitblocks: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitblocks: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x634af3921af234a34edacc85b951db4b2a9010df7362e9a6ee1c97d0b8f60e1f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CMC Alexandria 08/Feb/2021 Tesla Buys $1.5 Billion in Bitcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 150 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d4ad9f11ddd721e8da0cc9cfe7887e39e2de0b586538072c7ce6ab79612c85b5a4459af3bd8cb282d329b167c72e36ccee1ec735f669fd7572d3fb69e5c26828") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1613520897;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084716729;

        if (fTestNet)
        {
            block.nTime    = 1613520858;
            block.nNonce   = 385541801;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xa06af653605d69e4b4a1d41f1eac76e7771b33ea51c0e62230726264c7ea58f1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf5, 0xc6, 0xb7, 0xd8 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitblocksMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitblocksMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitblocksMiner(CWallet *pwallet)
{
    printf("BitblocksMiner started\n");
    ...
    try { loop {
        ...
        printf("Running BitblocksMiner with %" PRIszu " transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitblocksMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitblocksMiner, pwallet));
}
